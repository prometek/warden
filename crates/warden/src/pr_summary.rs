//! Composes the PR body handed to `warden-gated::pr_manager::finalize` as `summary_body` once a run
//! converges, including the Evidence section requires.

use warden_core::{EvidenceRow, Finding, RunState};

/// One cycle's findings, as already parsed/persisted by the orchestrator -- this module only
/// formats them, it never re-derives or re-validates finding content.
#[derive(Debug, Clone)]
...
/// Everything [`pr_body_from_run`] needs about the run itself, gathered by the caller from
/// `warden::db`.
#[derive(Debug, Clone)]
