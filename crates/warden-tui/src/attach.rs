//! Attaching to a run: merges `events` history with the live Event Bus stream so a late attach
//! shows the full history and then seamlessly switches to live, with no gap.
...
/// The result of attaching to a run: a model already reflecting every event known so far, plus (if
/// the run is still live) a channel to keep applying events from as they arrive.
pub struct Attachment {
    pub model: RunModel,
    /// `None` when the run has already finished (or was never live to begin with) and its Event Bus
    /// socket is gone.
...
/// `true` for the two ordinary ways a subscribe attempt fails when a run simply isn't live right
/// now -- see [`log_subscribe_failure`]'s docs.
