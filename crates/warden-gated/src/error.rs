...
#[derive(Debug, Error)]
pub enum GatedError {
    /// `warden-gated` must never create the database itself -- only `warden` does, via its
    /// migrations.
    ...
    /// A `post-receive` payload that isn't exactly `<old-sha> <new-sha> <ref-name>`, or whose ref
    /// doesn't match the gate's naming convention.
    ...
    /// The bare gate repo's `origin` remote isn't a recognizable GitHub URL -- most likely a
    /// misconfigured `origin`, or a non-GitHub remote with no explicit repo slug override supplied.
    #[error("could not determine a GitHub owner/repo from origin remote url {0:?}")]
    UnknownOriginRemote(String),

    /// `gh pr create`'s stdout didn't look like a PR URL ending in `/pull/<number>` -- an
    /// unexpected `gh` output format, surfaced rather than silently treated as "no PR number".
    ...
    /// `gh pr view --json state,statusCheckRollup`'s stdout didn't deserialize into the shape the
    /// CI watcher expects.
    #[error("could not parse gh pr view output as PR status ({reason}): {json:?}")]
    UnparsablePrStatusJson { json: String, reason: String },

    /// `gh`'s `state` field for a PR wasn't one of `OPEN`/`CLOSED`/`MERGED` -- a closed set per
    /// GitHub's own API, so anything else is a boundary error, not a guess.
    #[error("unknown PR lifecycle state from gh: {0:?}")]
    UnknownPrLifecycle(String),

    /// A `statusCheckRollup` entry's `status`/`conclusion` (Checks API) or `state` (legacy Statuses
    /// API) combination wasn't one this module recognizes.
    #[error("unknown CI check status/conclusion from gh: {0}")]
    UnknownCheckConclusion(String),

    /// A `statusCheckRollup` entry had neither the Checks API shape (`status`/`conclusion`) nor the
    /// legacy Statuses API shape (`state`).
    ...
    /// A `runs.pr_number` value that doesn't fit in a `u64`.
