//! GitHub implementation of the `PrProvider` seam, via the `gh` CLI.
...
/// GitHub PR provider: every call is scoped to one `owner/repo` via `gh`'s `--repo` flag, resolved
/// once at construction so individual PR actions never need to re-derive it.
...
impl CiProvider for GhProvider {
    /// Fetches `pr`'s lifecycle and CI check statuses in a single `gh` invocation.
...
struct RawCheckEntry {
    ...
    /// Checks API, only meaningful once `status == "COMPLETED"`: `"SUCCESS"` / `"FAILURE"` /
    /// `"NEUTRAL"` / `"SKIPPED"` / `"CANCELLED"` / `"TIMED_OUT"` / `"ACTION_REQUIRED"` / `"STALE"`.
    conclusion: Option<String>,
    /// Legacy Statuses API: `"SUCCESS"` / `"FAILURE"` / `"ERROR"` / `"PENDING"`.
    state: Option<String>,
...
/// Parses `gh pr view --json state,statusCheckRollup`'s stdout into a [`PrStatus`].
...
/// GitHub's `state` field is a closed set (`OPEN`/`CLOSED`/`MERGED`) -- any other value is a
/// boundary error, not a guess.
...
/// Classifies one `statusCheckRollup` entry, trying the Checks API shape (`status`/`conclusion`)
/// first and falling back to the legacy Statuses API shape (`state`).
...
/// Runs `gh <args>`, failing loudly (command, exit status, stderr attached) rather than swallowing
/// a non-zero exit.
...
/// Extracts `owner/repo` from a GitHub remote URL in any of the forms `git` itself commonly
/// produces (`https://`, `http://`, `git@host:`, `ssh://git@host/`).
